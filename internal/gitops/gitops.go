...
type Change struct {
	...
	// Domain is the assimilate-domain stamped into every written file's
	// marker. Stale files under cfg.Path carrying this domain are pruned;
	// files of other domains are never touched, and a target file of
	// another domain is a conflict.
	Domain string
	// Force overwrites files that fail the ownership check — files without
	// an assimilate marker, edited since assimilate wrote them, or generated
	// for another domain — and prunes stale files of this domain even when
...
// are overwritten — see writeFiles) and prunes the stale files of ch.Domain
// (see pruneFiles), commits on
...
// pushBranch is the git half: shallow-clone, apply ch under cfg.Path (write
// and prune), and — unless the tree is unchanged — commit on a fresh
...
func pushBranch(ctx context.Context, cfg spec.GitConfig, token string, ch Change, log func(string)) (branch string, noChanges bool, err error) {
	...
	if err := applyChange(dir, cfg, ch, log); err != nil {
		return "", false, err
	}
...
// applyChange writes ch.Files into the clone at dir and prunes the stale
// files of ch.Domain, both under cfg.Path. Every conflict either step finds
// is collected and reported in one error; with ch.Force there are none.
func applyChange(dir string, cfg spec.GitConfig, ch Change, log func(string)) error {
	conflicts, err := writeFiles(dir, cfg, ch, log)
	if err != nil {
		return err
	}
	pruned, err := pruneFiles(dir, cfg, ch, log)
	if err != nil {
		return err
	}
	conflicts = append(conflicts, pruned...)
	if len(conflicts) > 0 {
		return fmt.Errorf("refusing to touch files not generated by assimilate for this domain (re-run with --force to override):\n%s",
...
// ownership markers: YAML files gain a header of hash and domain comment
// lines, JSON files a sidecar holding the same. A destination that already
// exists but carries no marker (not generated by assimilate), whose body no
// longer matches its marker (edited since), or whose marker names another
// domain is a conflict: without ch.Force it is skipped and returned in the
// conflict list; with ch.Force the file is overwritten with a log note. A
// marker without a domain (written before domains existed) is adopted.
func writeFiles(dir string, cfg spec.GitConfig, ch Change, log func(string)) ([]string, error) {
	...
	for _, rel := range rels {
		...
		if reason := conflictReason(st, ch.Domain); st.Exists && reason != "" {
			if !ch.Force {
		...
		if err := ownership.WriteMarked(dst, ch.Files[rel], ch.Domain); err != nil {
			return nil, err
		}
	}
...
// by domain, or "" when it may: it carries no marker, its body was edited
// since the marker was written, or it belongs to another domain.
func conflictReason(st ownership.FileStatus, domain string) string {
	switch {
	case !st.Owned:
		return "no assimilate marker"
	case !st.Matches:
		return "edited since assimilate generated it"
	case st.Domain != "" && st.Domain != domain:
		return fmt.Sprintf("generated by assimilate-domain %q", st.Domain)
	}
	return ""
}
